...
//** GENERA LOS ASIENTOS LIBRERIA MD4 *//
//importamos las librerias principales *// 
//value for sun  a45rgts7qswmax
...
//--------------------------------------------- boque de codigo gen lib (MD-4) encargado de generar los acientos para el bus bus --------------------------------------------------- *//
...
<img src="images_bus/top_bus2.png">
</td>
</tr>
<tr class="bus">
<td class="bus" background="images_bus/background_bus1.png">
...
<tr class="bus">
<td class="bus" background="images_bus/background_bus1.png">
<img src="images_bus/pre_footer_bus.png">
</td>
</tr>
<tr class="bus">
<td class="bus" background="images_bus/background_bus1.png">
...
while ($a < 56){
	...
	$a++;	
	//funcion que efectua la consulta a la base de datos obteniendo el estado del asiento (libre, reservado, vendido) 
	check_status_places_b($bus, $dt, $a, $handle, $hrs, $branch);   //Grupo (b) aientos pares pasillo central izquierdo
	$a++;
	recover_info_backs_d();
	//funcion que efectua la consulta a la base de datos obteniendo el estado del asiento (libre, reservado, vendido
	check_status_places_d($bus, $dt, $a, $handle, $hrs, $branch); 	 //Grupo (c) aientos pares pasillo central derecho
	//Genera los textos para cada asiento	
	$b++;
	//funcion que efectua la consulta a la base de datos obteniedo el estado y el destino final de cada asiento (formato texto)
    ...
    $b++;
	recover_info_backs_c();
	//funcion que efectua la consulta a la base de datos obteniedo el estado y el destino final de cada asiento (formato texto)
	recover_info_destinations_d($bus, $dt, $b, $handle, $hrs, $branch);   //Grupo (c) asientos pares central pasillo derecho
...
//---------------------------------------------Fin del bloque----------------------------------------------------------------------------------------------------------------- *//
	  ...
	  mysqli_free_result($result);
		...
		echo "<script>
			function redirect()
			{
				window.location.replace('md-4.php');
			}
